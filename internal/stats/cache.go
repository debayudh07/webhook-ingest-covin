...
// ReplaceAll swaps the cache contents for a snapshot loaded from durable storage.
func (c *Cache) ReplaceAll(src map[string]AccountStats) {
	c.mu.Lock()
	defer c.mu.Unlock()

	c.m = make(map[string]*AccountStats, len(src))
	for id, st := range src {
		st := st
		c.m[id] = &st
	}
